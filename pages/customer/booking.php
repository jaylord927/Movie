            ...
            if ($schedule_result->num_rows === 0) {
                ...
                if ($seat_check_failed) {
                    ...
                    try {
                        ...
                        $booking_fee = count($selected_seats) * 350; // 350 per seat
                        ...
                        $selected_seats = []; // Reset selected seats
...
// If no movie selected or invalid, use first movie
...
// Get available schedules for selected movie
$schedules = [];
if ($movie_id > 0) {
...
// Get seat layout for selected schedule
...
<div class="main-container" style="max-width: 1400px; margin: 0 auto; padding: 20px;">
    <!-- Page Header with Movie Selection -->
                ...
                <form method="POST" action="" id="movieSelectForm">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <select name="movie_id" id="movieSelect" 
                                style="flex: 1; padding: 12px 15px; background: rgba(255,255,255,0.08);
                                       border: 2px solid rgba(226, 48, 32, 0.3); border-radius: 8px;
    ...
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 40px; min-height: 600px;">
        <!-- Left Column: Movie Details and Seat Selection -->
        <div>
            <!-- Movie Details Card -->
            <div style="background: linear-gradient(135deg, var(--bg-card) 0%, var(--bg-card-light) 100%); 
                 border-radius: 15px; padding: 25px; margin-bottom: 30px; 
            ...
            <div style="background: linear-gradient(135deg, var(--bg-card) 0%, var(--bg-card-light) 100%); 
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 30px; flex-wrap: wrap; padding: 15px; 
                    ...
                    <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <span style="color: rgba(255,255,255,0.9); font-size: 0.9rem; font-weight: 600;">Available</span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div style="width: 25px; height: 25px; background: #e23020; border-radius: 5px; border: 2px solid rgba(255,255,255,0.3);"></div>
                    ...
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div style="width: 25px; height: 25px; background: #17a2b8; border-radius: 5px; border: 2px solid rgba(255,255,255,0.3);"></div>
                        <span style="color: rgba(255,255,255,0.9); font-size: 0.9rem; font-weight: 600;">Premium</span>
                    </div>
                </div>
                ...
                <form method="POST" action="" id="bookingForm">
                    ...
                    <div style="margin-bottom: 40px;">
                        ...
                        $rows = [];
                        $premium_rows = ['A', 'B', 'C']; // Premium rows
                        ...
                        <div style="margin-bottom: 25px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 15px;">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    <div style="color: <?php echo $is_premium ? '#17a2b8' : 'white'; ?>; 
                                         font-weight: 700; font-size: 1.1rem; width: 40px; text-align: center;">
                                        <?php echo $row_letter; ?>
                                    </div>
                                    <?php if ($is_premium): ?>
                                    <span style="background: rgba(23, 162, 184, 0.2); color: #17a2b8; padding: 3px 10px; 
                            ...
                            <div style="display: flex; justify-content: center; gap: 12px; flex-wrap: wrap;">
                                ...
                                foreach ($row_seats as $col => $seat): 
                                    ...
                                    // Determine seat type
                                    $seat_type = 'standard';
                                    if ($is_premium) $seat_type = 'premium';
                                    if ($col >= 9 && $col <= 12) $seat_type = 'sweet-spot';
                                ...
                                <div style="position: relative;">
                                    ...
                                    <label style="cursor: <?php echo $seat_cursor; ?>; opacity: <?php echo $seat_opacity; ?>; display: block;">
                                        ...
                                        <div style="width: 40px; height: 45px; background: <?php echo $seat_bg; ?>; 
                                             ...
                                             onmouseover="hoverSeat(this, true)"
                                             onmouseout="hoverSeat(this, false)">
                                            <div style="font-size: 0.8rem;"><?php echo $row_letter; ?></div>
                                            <div style="font-size: 1rem;"><?php echo str_pad($col, 2, '0', STR_PAD_LEFT); ?></div>
                                            
                                            <?php if ($seat_type === 'sweet-spot' && $is_available): ?>
                                            <div style="position: absolute; top: -5px; right: -5px; background: #ffc107; 
                                                 color: #333; width: 15px; height: 15px; border-radius: 50%; font-size: 0.6rem; 
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <div id="summarySeats" style="color: white; font-weight: 700; min-height: 25px; font-size: 1.1rem;">
                            ...
                            <div style="display: flex; flex-wrap: wrap; gap: 5px;">
                                ...
                                <span style="background: rgba(226, 48, 32, 0.2); padding: 5px 10px; border-radius: 6px;">
                                    <?php echo $seat; ?>
                                </span>
                                <?php endforeach; ?>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBookingSummary() {
        const selectedSeats = Array.from(seatCheckboxes)
            ...
            .map(cb => ({
                number: cb.value,
                type: cb.dataset.seatType || 'standard'
            }));
        ...
        // Calculate total
        const total = (counts.standard * prices.standard) + 
                     (counts.premium * prices.premium) + 
                     (counts['sweet-spot'] * prices['sweet-spot']);
        ...
        if (selectedSeats.length > 0) {
            ...
            summarySeats.innerHTML = '<div style="display: flex; flex-wrap: wrap; gap: 5px;">' +
                seatNumbers.map(seat => 
                    `<span style="background: rgba(226, 48, 32, 0.2); padding: 5px 10px; border-radius: 6px;">${seat}</span>`
                ).join('') + '</div>';
        } else {
    ...
    seatCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            const seatType = this.dataset.seatType || 'standard';
            
            if (this.checked) {
                // Determine color based on seat type
                let bgColor = '#e23020'; // Default selected color
                
                seatDiv.style.background = bgColor;
                seatDiv.style.color = 'white';
                seatDiv.classList.add('seat-selected');
                seatDiv.classList.remove('seat-available', 'seat-premium', 'seat-sweet-spot');
                ...
                switch(seatType) {
                    ...
                    case 'sweet-spot':
                        bgColor = '#ffc107';
                        textColor = '#333';
                        seatDiv.classList.add('seat-sweet-spot');
                        break;
                    default:
                        bgColor = '#28a745';
                        textColor = 'white';
    ...
    // Initialize classes for available seats
    seatCheckboxes.forEach(checkbox => {
        if (!checkbox.disabled) {
            const seatDiv = checkbox.parentElement.querySelector('div');
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', function(e) {
            ...
            if (selectedSeats.length > 8) {
                e.preventDefault();
                showAlert('You can select maximum 8 seats per booking!', 'error');
                return false;
            }
    ...
    // Auto-submit schedule form when schedule is selected
...
// Add viewport scaling for seat layout
