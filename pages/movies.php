...
<div class="main-container" style="max-width: 1200px; margin: 0 auto; padding: 20px;">
    <div style="text-align: center; margin-bottom: 40px;">
        <h1 style="color: white; font-size: 2.5rem; margin-bottom: 15px; font-weight: 800;">All Movies</h1>
        <p style="color: var(--pale-red); font-size: 1.1rem; max-width: 600px; margin: 0 auto;">Browse our complete collection of movies and book your tickets</p>
    </div>
    ...
    <div style="background: linear-gradient(135deg, var(--bg-card) 0%, var(--bg-card-light) 100%); border-radius: 15px; padding: 30px; margin-bottom: 40px; border: 1px solid rgba(226, 48, 32, 0.2);">
        <div style="margin-bottom: 25px;">
            <form method="GET" action="">
                ...
                <div style="display: flex; gap: 10px; position: relative;">
                    ...
                    <a href="?page=movies" style="padding: 15px 20px; background: rgba(255,255,255,0.1); color: white; text-decoration: none; border-radius: 10px; border: 2px solid rgba(226, 48, 32, 0.3); display: flex; align-items: center; gap: 8px; font-weight: 600;"><i class="fas fa-times"></i> Clear</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
        ...
        <div style="margin-top: 25px; padding-top: 20px; border-top: 1px solid rgba(226, 48, 32, 0.2);">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 10px; align-items: center;">
                <?php if ($searchTerm): ?>
                <span style="background: linear-gradient(135deg, var(--primary-red) 0%, var(--dark-red) 100%); color: white; padding: 8px 16px; border-radius: 20px; font-size: 0.9rem; display: flex; align-items: center; gap: 8px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 4px 10px rgba(226, 48, 32, 0.2);">Search: "<?php echo htmlspecialchars($searchTerm); ?>" <a href="?page=movies<?php echo $selectedGenre ? '&genre=' . urlencode($selectedGenre) : ''; echo $selectedRating ? '&rating=' . urlencode($selectedRating) : ''; ?>" style="color: white; text-decoration: none; font-weight: 700; margin-left: 5px; padding: 2px 6px; background: rgba(0,0,0,0.2); border-radius: 50%;">×</a></span>
                <?php endif; ?>
                <?php if ($selectedGenre): ?>
                <span style="background: linear-gradient(135deg, var(--primary-red) 0%, var(--dark-red) 100%); color: white; padding: 8px 16px; border-radius: 20px; font-size: 0.9rem; display: flex; align-items: center; gap: 8px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 4px 10px rgba(226, 48, 32, 0.2);">Genre: <?php echo htmlspecialchars($selectedGenre); ?> <a href="?page=movies<?php echo $searchTerm ? '&search=' . urlencode($searchTerm) : ''; echo $selectedRating ? '&rating=' . urlencode($selectedRating) : ''; ?>" style="color: white; text-decoration: none; font-weight: 700; margin-left: 5px; padding: 2px 6px; background: rgba(0,0,0,0.2); border-radius: 50%;">×</a></span>
                <?php endif; ?>
                <?php if ($selectedRating): ?>
                <span style="background: linear-gradient(135deg, var(--primary-red) 0%, var(--dark-red) 100%); color: white; padding: 8px 16px; border-radius: 20px; font-size: 0.9rem; display: flex; align-items: center; gap: 8px; font-weight: 600; border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 4px 10px rgba(226, 48, 32, 0.2);">Rating: <?php echo htmlspecialchars($selectedRating); ?> <a href="?page=movies<?php echo $searchTerm ? '&search=' . urlencode($searchTerm) : ''; echo $selectedGenre ? '&genre=' . urlencode($selectedGenre) : ''; ?>" style="color: white; text-decoration: none; font-weight: 700; margin-left: 5px; padding: 2px 6px; background: rgba(0,0,0,0.2); border-radius: 50%;">×</a></span>
                <?php endif; ?>
    ...
    <?php if (empty($filteredMovies)): ?>
        <div style="text-align: center; padding: 60px; background: rgba(226, 48, 32, 0.05); border-radius: 15px; border: 2px dashed rgba(226, 48, 32, 0.3);">
            ...
            <p style="color: var(--pale-red); margin-bottom: 25px; max-width: 400px; margin-left: auto; margin-right: auto;">We couldn't find any movies matching your criteria.</p>
            <a href="?page=movies" class="btn btn-primary" style="padding: 12px 30px;"><i class="fas fa-times"></i> Clear All Filters</a>
        </div>
    <?php else: ?>
        <div class="movies-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 30px;">
            ...
            <div class="movie-card" style="background: linear-gradient(135deg, var(--bg-card) 0%, var(--bg-card-light) 100%); border-radius: 15px; overflow: hidden; transition: all 0.3s ease; border: 1px solid rgba(226, 48, 32, 0.2); position: relative; display: flex; flex-direction: column; height: 100%;">
                <?php if (!empty($movie['poster_url'])): ?>
                    <img src="<?php echo $movie['poster_url']; ?>" alt="<?php echo htmlspecialchars($movie['title']); ?>" style="width: 100%; height: 320px; object-fit: cover;" onerror="this.onerror=null; this.src='data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 300 400\"><rect width=\"300\" height=\"400\" fill=\"%233a0b07\"/><text x=\"50%\" y=\"50%\" font-family=\"Arial\" font-size=\"24\" fill=\"%23ff9999\" text-anchor=\"middle\" dy=\".3em\">No Poster</text></svg>'">
                <?php else: ?>
                    <div style="width: 100%; height: 320px; background: linear-gradient(135deg, rgba(226, 48, 32, 0.1), rgba(193, 27, 24, 0.2)); display: flex; align-items: center; justify-content: center;"><i class="fas fa-film" style="font-size: 3rem; color: rgba(255, 255, 255, 0.3);"></i></div>
                <?php endif; ?>
                ...
                <div style="padding: 25px; flex: 1; display: flex; flex-direction: column;">
                    <h3 style="color: white; font-size: 1.3rem; font-weight: 800; margin-bottom: 15px; line-height: 1.4; min-height: 3.2em; flex-shrink: 0;"><?php echo htmlspecialchars($movie['title']); ?></h3>
                    <div style="margin-bottom: 15px; flex-shrink: 0;">
                        <?php if ($movie['duration']): ?>
                        <div style="display: flex; align-items: center; gap: 8px; color: rgba(255,255,255,0.8); font-size: 0.9rem; margin-bottom: 8px;"><i class="fas fa-clock"></i> <?php echo $movie['duration']; ?></div>
                        <?php endif; ?>
                        <?php if ($movie['genre']): ?>
                    ...
                    <div style="flex: 1; margin-bottom: 20px;">
                        <p style="color: rgba(255,255,255,0.7); font-size: 0.95rem; line-height: 1.6; max-height: 4.5em; overflow: hidden; position: relative;"><?php echo substr(htmlspecialchars($movie['description']), 0, 100); ?><?php if (strlen($movie['description']) > 100): ?>...<?php endif; ?></p>
                    </div>
                    <?php endif; ?>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: auto; flex-shrink: 0;">
                        <a href="<?php echo SITE_URL; ?>index.php?page=customer/movie-details&id=<?php echo $movie['id']; ?>" class="btn btn-secondary" style="padding: 12px; text-align: center; font-size: 0.9rem; height: 44px; display: flex; align-items: center; justify-content: center; gap: 8px;"><i class="fas fa-info-circle"></i> Details</a>
                        <?php if (isset($_SESSION['user_id']) && $_SESSION['user_role'] === 'Customer'): ?>
                            <a href="<?php echo SITE_URL; ?>index.php?page=customer/booking&movie=<?php echo $movie['id']; ?>" class="btn btn-primary" style="padding: 12px; text-align: center; font-size: 0.9rem; height: 44px; display: flex; align-items: center; justify-content: center; gap: 8px;"><i class="fas fa-ticket-alt"></i> Book Now</a>
                        <?php elseif (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'Admin'): ?>
                            <a href="<?php echo SITE_URL; ?>index.php?page=admin/dashboard" class="btn btn-primary" style="padding: 12px; text-align: center; font-size: 0.9rem; height: 44px; display: flex; align-items: center; justify-content: center; gap: 8px;"><i class="fas fa-shield-alt"></i> Admin</a>
                        <?php else: ?>
                            <a href="<?php echo SITE_URL; ?>index.php?page=login" class="btn btn-primary" style="padding: 12px; text-align: center; font-size: 0.9rem; height: 44px; display: flex; align-items: center; justify-content: center; gap: 8px;"><i class="fas fa-sign-in-alt"></i> Login to Book</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <div style="text-align: center; margin-top: 40px; padding: 20px; background: rgba(226, 48, 32, 0.05); border-radius: 10px; border: 1px solid rgba(226, 48, 32, 0.2);">
            <p style="color: var(--pale-red); font-size: 1.1rem;">Showing <strong style="color: white;"><?php echo count($filteredMovies); ?></strong> movies <?php if ($searchTerm || $selectedGenre || $selectedRating): ?>(filtered from <?php echo count($movies); ?> total movies)<?php else: ?>in our collection<?php endif; ?></p>
        </div>
    <?php endif; ?>
</div>
